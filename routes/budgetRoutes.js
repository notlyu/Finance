const express = require('express');
const authMiddleware = require('../middleware/auth');
const budgetController = require('../controllers/budgetController');
const { validateMiddleware, validateObjectId, validateQuery } = require('../lib/validation');

const router = express.Router();

router.use(authMiddleware);

router.get('/', validateQuery('budgets'), budgetController.getBudgets);
router.post('/', validateMiddleware('budget', 'create'), budgetController.createBudget);
router.patch('/:id', validateObjectId, validateMiddleware('budget', 'update'), budgetController.updateBudget);
router.delete('/:id', validateObjectId, budgetController.deleteBudget);

module.exports = router;

