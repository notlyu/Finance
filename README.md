# 💰 Finance — Личный финансовый трекер

Приложение для управления личными финансами: учёт доходов/расходов, цели, желания, бюджеты, recurring-транзакции, подушка безопасности, долги, импорт/экспорт, аналитика и настраиваемая приборная панель.

> ℹ️ Семейный режим (общие финансы на несколько человек) присутствует в коде, но **отключён по умолчанию** — приложение работает как **личный** трекер. Включить: `REACT_APP_FAMILY_ENABLED=true`.
...
- Cookie-only сессии + CSRF-защита
- Регистрация, вход, выход
- Сброс пароля через email (SMTP/Nodemailer)

> 👨‍👩‍👧‍👦 **Семейный режим** (общие финансы, приглашения, роли Owner/Member) — опциональный, **отключён по умолчанию**. Включается флагом `REACT_APP_FAMILY_ENABLED=true`.
...
| `Family` *(опц.)* | Семья (название, код приглашения) |
| `FamilyMember` *(опц.)* | Член семьи (роль: Owner/Member) |
| `FamilyInvite` *(опц.)* | Приглашения (код, срок действия) |
| `Account` | Финансовый счёт (тип, баланс, валюта) |
| `Transaction` | Транзакция (сумма, тип, категория, счёт) |
...
> Эндпоинты доступны на бэкенде, но семейный режим в UI **отключён по умолчанию** (`REACT_APP_FAMILY_ENABLED`).
