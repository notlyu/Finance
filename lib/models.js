...
const models = {
  ...
  Transaction: {
    ...
    sum: async (field, options = {}) => {
      ...
      return agg._sum && agg._sum[field] != null ? agg._sum[field] : 0;
    },
  },
  Category: {
    findOne: async (options) => {
    ...
    findOrCreate: async (options) => {
      const where = parseWhere(options.where || {}, 'Category');
      let result = await prisma.category.findFirst({ where });
      if (!result && options.defaults) {
        result = await prisma.category.create({ data: { ...options.where, ...options.defaults } });
      }
      return result ? [result, result.isNewRecord || false] : [result, true];
    },
    findByPk: async (id, options = {}) => {
  ...
  Goal: {
    findOne: async (options) => {
      ...
      const result = await prisma.goal.findFirst({ where, include: options.include ? parseInclude(options.include) : undefined });
      return result ? addUpdateMethod(result, 'Goal') : result;
    },
  ...
  Wish: {
    findOne: async (options) => {
      ...
      const result = await prisma.wish.findFirst({ where, include: options.include ? parseInclude(options.include) : undefined });
      return result ? addUpdateMethod(result, 'Wish') : result;
    },
    findByPk: async (id) => prisma.wish.findUnique({ where: { id } }),
    create: (data) => {
      const result = prisma.wish.create({ data });
      return result.then(r => addUpdateMethod(r, 'Wish'));
    },
    update: async (options) => {
      const { where, ...rest } = options;
