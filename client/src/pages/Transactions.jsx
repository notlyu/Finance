    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Скрытые операции</label>
            <select
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {transactions.map(t => (
                <tr key={t.id} className={t.is_hidden ? 'bg-gray-50' : ''}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{t.comment || '-'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{t.user_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    {!t.is_hidden && (
                      <>
                        <button onClick={() => openEditModal(t)} className="text-indigo-600 hover:text-indigo-900 mr-2">✏️</button>
              ...
              {transactions.length === 0 && (
                <tr>
                  <td colSpan="6" className="px-6 py-4 text-center text-gray-500">Нет операций</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Редактировать операцию' : 'Добавить операцию'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <select {...register('type', { required: true })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
          ...
          <div>
            ...
            <select {...register('category_id', { required: true })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
          ...
          <div>
            ...
            <input type="date" {...register('date', { required: true })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Комментарий</label>
            <textarea {...register('comment')} rows="2" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" />
          </div>
          ...
          <div className="flex justify-end space-x-2 pt-4">
            <button type="button" onClick={() => setModalOpen(false)} className="px-4 py-2 border rounded-md text-gray-700 hover:bg-gray-50">Отмена</button>
