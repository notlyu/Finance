...
export default function Family() {
  ...
  if (loading) return (
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <SkeletonList items={4} className="w-full max-w-xl" />
    </div>
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex gap-2">
        <button onClick={() => setActiveTab('members')} className={`px-5 py-2.5 rounded-3xl text-sm font-bold transition-colors ${activeTab === 'members' ? 'bg-primary text-white' : 'bg-surface-container text-on-surface-variant hover:bg-surface-container-high'}`}>
          Участники
        </button>
        <button onClick={() => setActiveTab('finance')} className={`px-5 py-2.5 rounded-3xl text-sm font-bold transition-colors ${activeTab === 'finance' ? 'bg-primary text-white' : 'bg-surface-container text-on-surface-variant hover:bg-surface-container-high'}`}>
          Финансы
        </button>
      </div>
      ...
      <div className="bg-surface-container-lowest p-8 rounded-3xl shadow-card">
        ...
        <div className="flex items-center gap-3 p-4 bg-surface-container rounded-3xl">
          <p className="text-sm text-on-surface-variant">Приглашайте участников, создавая временные коды в разделе «Участники»</p>
        </div>
      </div>
            ...
            <div className="bg-surface-container p-6 rounded-3xl">
              ...
              {invites.length > 0 ? (
                <div className="space-y-3">
                  {invites.map(inv => (
                    <div key={inv.id} className="flex items-center justify-between gap-3 p-4 bg-surface-container-lowest rounded-3xl">
                ...
                <EmptyState title="Нет активных приглашений" />
              )}
            </div>
          )}
          ...
          <div className="bg-surface-container-lowest p-6 rounded-3xl shadow-card">
            ...
            <div className="space-y-3">
              {family.members?.map(member => {
                ...
                return (
                  <div key={member.id} className="flex justify-between items-center p-4 bg-surface-container rounded-3xl">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isOwner && !isMemberOwner && (
                        <button onClick={() => removeMember(member.id, member.name)} className="w-9 h-9 flex items-center justify-center rounded-xl text-error hover:bg-error-container transition-colors" title="Удалить">
      ...
      {activeTab === 'finance' && (
        <div className="space-y-6">
          <div className="bg-surface-container-lowest p-6 rounded-3xl shadow-card">
            ...
            {memberStats.length > 0 ? (
              <div className="space-y-4">
                {memberStats.map((m, idx) => {
                  ...
                  return (
                    <div key={m.userId} className="bg-surface-container p-4 rounded-3xl">
            ...
            <EmptyState title="Нет данных о транзакциях" />
            )}
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            {family.members?.filter(m => m.id !== user.id).map(m => (
              <button key={m.id} onClick={() => setTransferTarget(m)} className={`w-full text-left p-4 rounded-3xl border-2 transition ${
      ...
      {promptModal.open && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-surface-container-lowest rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
